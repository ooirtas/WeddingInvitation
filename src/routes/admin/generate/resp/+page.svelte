<script>
  ...
  import {
    ...
    buildResepsiWhatsappMessage
  } from '$lib/utils/invitation';

  let baseDomain = 'https://kaliefinvitation.com';
  let rawGuests = 'Pak Yoto & Tante Yuni\nIbu Lestari\nMas Rian & Keluarga';
  let copied = '';
  ...
  $: rows = guests.map((guest) => {
    const link = buildResepsiInvitationLink(baseDomain, guest);
    const message = buildResepsiWhatsappMessage(baseDomain, guest);
  ...
  function exportToExcel() {
    ...
    link.href = url;
    link.setAttribute('download', 'guest_list_resepsi.csv');
...
<svelte:head>
  <title>Admin Generator (Resepsi) | {siteConfig.title}</title>
</svelte:head>

<div class="shell py-10 sm:py-14">
  <div class="mx-auto max-w-5xl">
    <div class="text-center">
      <p class="section-tag">Admin Utility (Resepsi)</p>
      <h1 class="section-title mt-5">Generate personalized invitation links</h1>
      <div class="soft-divider"></div>
      <p class="section-copy mx-auto max-w-2xl">
        Tempel daftar nama tamu, lalu sistem akan membuat link undangan resepsi personal, pesan WhatsApp,
    ...
    <section class="mt-10 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <div class="glass-card rounded-[2rem] p-6 sm:p-8">
        <label class="grid gap-2 text-sm font-medium text-cocoa">
          Base Domain
          <input bind:value={baseDomain} class="rounded-[1.2rem] border border-gold/15 bg-white/70 px-4 py-3 outline-none focus:border-gold" placeholder="https://kaliefinvitation.com" />
        </label>
